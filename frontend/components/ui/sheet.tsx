...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    ref={ref}
    className={cn(
      // Velo liviano azul-gris (no negro puro — misma filosofía que las sombras):
      // el blur ya separa la capa, el contexto queda visible detrás.
...
>(({ side = "right", className, children, hideClose, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      {!hideClose && (
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-md p-1 text-muted-foreground opacity-80 ring-offset-background transition-opacity hover:opacity-100 hover:bg-muted focus:outline-none focus:ring-2 focus:ring-ring disabled:pointer-events-none">
          <X className="h-4 w-4" />
