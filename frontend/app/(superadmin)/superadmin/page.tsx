...
import {
  Shield, Plus, Loader2, RefreshCw, Users, FileText, TrendingUp,
  ChevronDown, ChevronRight, PauseCircle, PlayCircle, Settings2
} from "lucide-react";
...
export default function SuperAdminPage() {
  ...
  const suspendM = useMutation({
    ...
    onSuccess: () => { inv(); toast({ title: "Tenant suspendido", variant: "default" }); },
    onError: () => toast({ title: "Error al suspender", variant: "destructive" }),
  });
  const activateM = useMutation({
    mutationFn: tenantsApi.activate,
    onSuccess: () => { inv(); toast({ title: "Tenant reactivado", variant: "success" }); },
  ...
  const updateM = useMutation({
    ...
    onSuccess: () => { inv(); toast({ title: "Plan actualizado", variant: "success" }); },
    onError: () => toast({ title: "Error al actualizar", variant: "destructive" }),
  });

  const activeCount    = tenants.filter(t => t.status === "active").length;
  const totalQueries   = tenants.reduce((s, t) => s + (t.usage_30d?.queries || 0), 0);
    ...
    <div className="min-h-screen bg-muted/30">
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <StatCard icon={TrendingUp} label="Consultas (30d)" value={totalQueries.toLocaleString()} />
        </div>

        {/* Lista de tenants */}
        <Card>
          <CardHeader className="pb-3">
            <h2 className="font-semibold text-sm">Tenants ({tenants.length})</h2>
          </CardHeader>
          <CardContent>
            {isLoading ? (
...
// ── Tenant row ────────────────────────────────────────────────────────────────
function TenantRow({ tenant: t, onSuspend, onActivate, onChangePlan }: {
  tenant: TenantRow;
  ...
  return (
    <div className="rounded-lg border overflow-hidden">
      <div
        className="flex items-center gap-3 p-3 hover:bg-accent/20 cursor-pointer transition-colors"
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${PLAN_COLORS[t.plan] || "bg-muted"}`}>
            {t.plan}
          </span>
          ...
          <span className="text-xs text-muted-foreground hidden sm:block">
            {t.usage_30d.queries.toLocaleString()} q/30d
          </span>
        </div>
      </div>
...
function CreateTenantModal({ open, onClose, onCreated }: {
  ...
  const createM = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Tenant creado", description: `'${form.id}' provisionado correctamente.`, variant: "success" });
    },
    onError: (err: any) => {
      const msg = err?.response?.data?.detail ?? "Error al crear el tenant.";
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3 py-2">
          {[
            ...
            <div key={f.key} className="space-y-1">
              ...
              <Input type={f.type} placeholder={f.placeholder} value={(form as any)[f.key]}
                onChange={set(f.key)} className="h-8 text-sm" />
            </div>
          ))}
          <div className="space-y-1">
            <Label className="text-xs">Plan</Label>
            <select
              className="w-full text-sm border rounded px-3 py-1.5 bg-background h-8"
          ...
          <p className="text-xs text-muted-foreground">
            El ID debe ser único y se usa como nombre de schema en PostgreSQL y subdominio. Solo minúsculas, números y guiones.
          </p>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>Cancelar</Button>
          <Button
            disabled={createM.isPending || !form.id || !form.admin_email || !form.admin_password}
