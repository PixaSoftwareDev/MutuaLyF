...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // NOTE: Role extraction from JWT — decode payload without verification (already validated by server)
      const payload = JSON.parse(atob(data.access_token.split(".")[1]));
      setAuth(data.access_token, tenantId, email, payload.role ?? "user");
      router.push("/dashboard");
    } catch (err: any) {
      const detail = err?.response?.data?.detail;
      setError(typeof detail === "string" ? detail : "Credenciales incorrectas. Verificá tu email, contraseña y tenant.");
    } finally {
      setLoading(false);
    }
  };
