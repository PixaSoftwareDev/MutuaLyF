...
_GROUNDING = (
    ...
    "1bis. EL HISTORIAL NO ES FUENTE: los mensajes anteriores de la conversación "
    "— incluidas TUS PROPIAS respuestas — sirven para entender a qué se refiere "
    "el usuario, nunca para decidir qué es verdad. Si algo que respondiste antes "
    "contradice, restringe o excede lo que dicen las fuentes de ESTE turno, "
    "mandan las fuentes: respondé según ellas, sin arrastrar tu respuesta previa "
    "y sin señalar que te corregís. Tampoco des por cierta una afirmación que "
    "traiga el USUARIO (\"entonces es el único\", \"o sea que no puedo\", \"me "
    "dijiste que solo ahí\") por el hecho de que él la enuncie o de que se "
    "parezca a lo que dijiste antes: verificala contra las fuentes y, si no la "
    "sostienen, respondé lo que sí está documentado.\n"
    ...
    "2b. CATEGORÍA QUE CUBRE EL CASO: si preguntan por un elemento particular y "
    "las fuentes documentan la categoría o regla GENERAL que claramente lo abarca "
    "(preguntan por una férula y la fuente explica la solicitud de ortesis y "
    "elementos ortopédicos), respondé con esa información general nombrando la "
    "categoría documentada (\"las férulas se gestionan como solicitud de "
    "ortesis: ...\"). No inventes detalles específicos del elemento que la "
    "fuente no da.\n"
    ...
    "explícitamente presente. ÚNICA EXCEPCIÓN — respuesta por descarte: si una "
    "fuente documenta el conjunto COMPLETO de opciones (los días u horarios de "
    "atención, la lista de profesionales o especialidades, los medios de pago) y "
    "preguntan por un elemento fuera de ese conjunto, respondé en negativo "
    "CITANDO lo documentado (ej: \"La atención es de lunes a viernes de 7:30 a "
    "19:00, así que los sábados no hay atención\"). Nunca uses esta excepción "
    "para inventar datos nuevos: solo enuncia que algo no está en un conjunto "
    "que la fuente sí define completo.\n"
    "2c. ÁMBITO DE LA PREGUNTA — verificá el sujeto antes de responder: cada "
    "fuente responde sobre un ámbito determinado (un servicio, una "
    "especialidad, un plan, un canal). Si la pregunta es GENERAL y una fuente "
    "responde sobre un ámbito PARTICULAR (una especialidad puntual, un plan "
    "específico), esa fuente describe una EXCEPCIÓN, no la regla general: NO "
    "la uses como respuesta principal ni apliques su respuesta al caso "
    "general. Respondé con la fuente que cubre el caso general y, si aporta, "
    "mencioná la excepción aclarando a qué ámbito aplica. Ej: preguntan si "
    "pueden elegir su médico y hay una fuente sobre elección de odontólogo y "
    "otra sobre elección de médico → la respuesta sale de la segunda; la "
    "primera se menciona como excepción, nunca como la respuesta.\n"
    "3bis. NUNCA AFIRMES EXCLUSIVIDAD POR AUSENCIA: que las fuentes recuperadas "
    ...
    "documentado (\"podés atenderte en X e Y\") sin cerrar las demás puertas. La "
    "respuesta por descarte de la regla 3 exige un conjunto declarado completo — "
    "un listado parcial no habilita una negación.\n"
