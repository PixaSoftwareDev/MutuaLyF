...
_JUDGE_SYSTEM = (
    ...
    "información NECESARIA para responder la pregunta — sea en forma directa "
    "o POR DESCARTE (regla 3c). Reglas:\n"
    ...
    "responde.\n"
    "3c. POR DESCARTE — MUY IMPORTANTE: cuando un fragmento ENUMERA el "
    "conjunto completo de opciones disponibles (los días u horarios en que "
    "atiende un profesional o servicio, la lista de profesionales de una "
    "especialidad, los medios de pago habilitados) y la pregunta es por una "
    "opción que NO figura en esa enumeración, el fragmento SÍ RESPONDE la "
    "pregunta: la respuesta es que esa opción no está disponible. PROHIBIDO "
    "responder 'no se menciona X' cuando un fragmento enumera las opciones y "
    "X simplemente no está entre ellas — esa enumeración ES la respuesta. "
    "Ej: preguntan '¿atienden los sábados?' y un fragmento dice 'atiende los "
    "miércoles de 14 a 15:30' → ese fragmento RESPONDE (los sábados no "
    "atiende, porque solo atiende miércoles).\n"
    "4. Ante la duda sobre un fragmento puntual que NO enumera un conjunto, "
    "NO responde. Si ninguno responde ni en parte ni por descarte, la lista "
...
def coverage_note(missing: list[str]) -> str:
    ...
    return (
        ...
        "normalmente e ignorá esta nota. OJO: que una palabra de la lista no "
        "aparezca literal NO significa que falte la respuesta — si los "
        "fragmentos documentan el conjunto completo de opciones (días, "
        "horarios, listas), la pregunta por un elemento fuera del conjunto "
        "queda respondida por descarte citando lo documentado (ej: 'la "
        "atención es de lunes a viernes' responde si atienden los sábados). "
        "Lo mismo si los fragmentos documentan la categoría general que abarca "
        "lo preguntado (una férula es una ortesis): respondé con esa "
        "información general nombrando la categoría. Solo si la pregunta pide "
        "específicamente algo de esa lista y no está en los fragmentos, ni por "
        "descarte, ni cubierto por una categoría documentada, aclaralo "
        "diciendo que NO TENÉS INFORMACIÓN sobre eso "
        "(nunca afirmes "
